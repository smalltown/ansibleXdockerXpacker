...
mount_app_path=${mount_app_path:-"/tmp/app_repo"}
...
docker run --detach --volume="$PWD":/etc/ansible/roles/role_under_test:rw --volume="$app_path":"$mount_app_path":ro  --publish 80:80 --name $container_id $opts geerlingguy/docker-$distro-ansible:latest $init
